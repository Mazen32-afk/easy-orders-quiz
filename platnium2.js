...
function extractProductData(doc,fallbackData,slug){var productObject=getProductObjectFromSlugPage(doc,slug);var title=getTitleFromProductObject(productObject)||getMetaContent(doc,'meta[property="og:title"]')||getMetaContent(doc,'meta[name="twitter:title"]')||getText(doc,"h1")||fallbackData.title;var image=getImageFromProductObject(productObject)||getMetaContent(doc,'meta[property="og:image"]')||getMetaContent(doc,'meta[name="twitter:image"]')||getImageFromPage(doc);var price=getFinalPriceFromProductSlug(doc,slug,productObject);return{title:cleanTitle(title),image:image||"",price:price||""}}
...
function getImageFromProductObject(obj){if(!obj||typeof obj!=="object")return "";var keys=["image","image_url","imageUrl","thumbnail","thumbnail_url","thumbnailUrl","featured_image","featuredImage"];for(var i=0;i<keys.length;i++){var v=obj[keys[i]];if(typeof v==="string"&&v)return abs(v);if(v&&typeof v==="object"){if(typeof v.url==="string")return abs(v.url);if(typeof v.src==="string")return abs(v.src)}}if(Array.isArray(obj.images)&&obj.images.length){var first=obj.images[0];if(typeof first==="string")return abs(first);if(first&&typeof first.url==="string")return abs(first.url);if(first&&typeof first.src==="string")return abs(first.src)}return ""}
