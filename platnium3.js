...
function getMainProductImage(doc,productObject){return getImageFromProductObject(productObject)||getSafeMetaImage(doc)||""}
...
function getImageFromPage(doc){var selectors=["[data-product-image] img","[data-main-image] img","[data-featured-image] img",".product-gallery img",".product__media img",".product img",".product-page img",".swiper-slide img","img[src*='digitaloceanspaces']","img"];var fallback="";for(var i=0;i<selectors.length;i++){var imgs=doc.querySelectorAll(selectors[i]);for(var j=0;j<imgs.length;j++){var img=imgs[j];if(hasVideoParent(img))continue;var src=img.getAttribute("src")||img.getAttribute("data-src")||img.getAttribute("data-lazy-src")||img.getAttribute("data-original")||img.getAttribute("data-zoom-image")||getSrcFromSrcset(img.getAttribute("srcset")||img.getAttribute("data-srcset")||"")||"";var image=cleanImageUrl(src);if(!image)continue;if(!isWeakImageUrl(image))return image;if(!fallback)fallback=image}}return fallback}
