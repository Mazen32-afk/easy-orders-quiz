(function(){
var GAME_KEY="__LIMITLESS_LAST_SHIPPING_GAME_STATE__";
var GAME_STATE=window[GAME_KEY]=window[GAME_KEY]||{};
if(GAME_STATE.loaded&&typeof GAME_STATE.remount==="function"){
  GAME_STATE.remount();
  return;
}
...
.grn{color:#22c55e!important;font-weight:900!important;text-shadow:0 0 14px rgba(34,197,94,.35)}
...
var oldStyle=document.getElementById("LimitlessQuizGameStyle");
...
function ask(){lock=false;var q=qs[step],arr=q.o.map(function(x,i){return{text:x,ok:i==q.a}});arr=Sh(arr);q.cur=arr;var L=["A","B","C","D"],h="";for(var i=0;i<arr.length;i++)h+='<button class="op" data-i="'+i+'"><span class="lt">'+L[i]+'</span><strong>'+markImportant(arr[i].text)+'</strong></button>';openBox('<div class="meta"><div class="pill">السؤال '+(step+1)+' من 3</div></div><div class="timer" id="tm">⏱ '+time+'s</div><div class="bar"><i style="width:'+((step/3)*100+15)+'%"></i></div><h3 class="q">'+markImportant(q.q)+'</h3><div class="ops">'+h+'</div>'+cpn());paintTime();document.querySelectorAll("#pqo .op").forEach(function(b){b.onclick=function(){ans(+this.getAttribute("data-i"),this)}})}
...
function wrong(msg){clearInterval(timer);openBox('<div class="ic e">×</div><h2 class="ttl">للأسف إجابتك خاطئة</h2><p class="ds">'+markImportant(msg)+'</p><div class="act"><button class="btn p" id="retry">إعادة التحدي</button><a class="btn wa" href="'+WA+'" target="_blank">التواصل مع د/مازن</a></div>');G("retry").onclick=intro}
function win(){clearInterval(timer);var o=C[code];openBox('<div class="ic">✓</div><h2 class="ttl">تم فتح <span>قسيمة الخصم</span></h2><p class="ds">'+markImportant(o.msg)+'</p><div class="code">'+code+'</div><div class="cd">هذه القسيمة صالحة لمدة 24 ساعة: <span id="cdt">24:00:00</span></div><div class="act"><button class="btn p" id="copyc">نسخ القسيمة</button><button class="btn" id="shopc">ابدأ التسوق الآن</button></div><div class="pg" id="plist"></div>');G("copyc").onclick=function(){navigator.clipboard&&navigator.clipboard.writeText(code);this.innerHTML="تم النسخ"};G("shopc").onclick=function(){location.href=o.p[0]?productUrl(o.p[0]):"/collections/all"};count(86400);cards(code)}
...
async function loadProductData(product){var slug=String(product.slug||"").trim();if(productCache[slug])return productCache[slug];var fallback={title:product.title||slug,image:"",price:""};try{var response=await fetch(productUrl(slug),{method:"GET",credentials:"same-origin"});if(!response.ok){productCache[slug]=fallback;return fallback}var html=await response.text();var doc=new DOMParser().parseFromString(html,"text/html");var data=extractProductData(doc,fallback,slug);productCache[slug]=data;return data}catch(e){productCache[slug]=fallback;return fallback}}
function extractProductData(doc,fallbackData,slug){var productObject=getProductObjectFromSlugPage(doc,slug);var title=getTitleFromProductObject(productObject)||getMetaContent(doc,'meta[property="og:title"]')||getMetaContent(doc,'meta[name="twitter:title"]')||getText(doc,"h1")||fallbackData.title;var image=getMainProductImage(doc,productObject)||getImageFromPage(doc);var price=getFinalPriceFromProductSlug(doc,slug,productObject);return{title:cleanTitle(title),image:image||"",price:price||""}}
function getProductObjectFromSlugPage(doc,slug){var next=doc.querySelector("script#__NEXT_DATA__");if(!next)return null;try{var data=JSON.parse(next.textContent);return findObjectBySlug(data,slug)}catch(e){return null}}
function findObjectBySlug(node,slug){if(!node)return null;if(Array.isArray(node)){for(var i=0;i<node.length;i++){var found=findObjectBySlug(node[i],slug);if(found)return found}return null}if(typeof node==="object"){if(objectMatchesSlug(node,slug))return node;for(var key in node){if(Object.prototype.hasOwnProperty.call(node,key)){var foundChild=findObjectBySlug(node[key],slug);if(foundChild)return foundChild}}}return null}
function objectMatchesSlug(obj,slug){var wanted=normalizeSlug(slug);var keys=["slug","handle","product_slug","productSlug","url","href","path","permalink"];for(var i=0;i<keys.length;i++){var value=obj[keys[i]];if(typeof value==="string"&&normalizeSlug(value)===wanted)return true}return false}
function normalizeSlug(value){var s=String(value||"").trim();try{s=decodeURIComponent(s)}catch(e){}s=s.split("?")[0].split("#")[0];if(s.indexOf("/products/")>-1)s=s.split("/products/")[1];s=s.replace(/^\/+|\/+$/g,"");if(s.indexOf("/")>-1){var parts=s.split("/");s=parts[parts.length-1]}return s.toLowerCase()}
function getTitleFromProductObject(obj){if(!obj||typeof obj!=="object")return "";return obj.title||obj.name||obj.product_title||obj.productTitle||""}
function getMainProductImage(doc,productObject){return getEasyOrdersMainGalleryImage(doc)||getImageFromProductObject(productObject)||getSafeMetaImage(doc)||""}
function getEasyOrdersMainGalleryImage(doc){var selectors=["img#gallery-main-image.ab-gallery-main-media","img#gallery-main-image",".ab-gallery-slide.is-active img.ab-gallery-main-media",".ab-gallery-slide.is-active img",".ab-gallery-main img.ab-gallery-main-media",".ab-gallery-main img","[data-ab-gallery-slider] .ab-gallery-slide.is-active img"];var fallback="";for(var i=0;i<selectors.length;i++){var imgs=doc.querySelectorAll(selectors[i]);for(var j=0;j<imgs.length;j++){var image=getImageFromImgElement(imgs[j]);if(!image)continue;if(!isWeakImageUrl(image))return image;if(!fallback)fallback=image}}var slideSelectors=[".ab-gallery-slide.is-active[data-src]",".ab-gallery-slide[data-slide-index='0'][data-src]",".ab-gallery-slides .ab-gallery-slide[data-src]"];for(var k=0;k<slideSelectors.length;k++){var slides=doc.querySelectorAll(slideSelectors[k]);for(var m=0;m<slides.length;m++){if(hasVideoParent(slides[m]))continue;var slideImage=cleanImageUrl(slides[m].getAttribute("data-src")||"");if(!slideImage)continue;if(!isWeakImageUrl(slideImage))return slideImage;if(!fallback)fallback=slideImage}}return fallback}
function getImageFromImgElement(img){if(!img)return "";if(hasVideoParent(img))return "";var src=img.getAttribute("src")||img.getAttribute("data-src")||img.getAttribute("data-lazy-src")||img.getAttribute("data-original")||img.getAttribute("data-zoom-image")||img.getAttribute("data-large-image")||img.getAttribute("data-full")||getSrcFromSrcset(img.getAttribute("srcset")||img.getAttribute("data-srcset")||"")||"";var image=cleanImageUrl(src);if(image)return image;var parent=img.closest?img.closest("[data-src]"):null;return parent?cleanImageUrl(parent.getAttribute("data-src")||""):""}
function getImageFromProductObject(obj){if(!obj||typeof obj!=="object")return "";var mainKeys=["featured_image","featuredImage","main_image","mainImage","primary_image","primaryImage","cover_image","coverImage","product_image","productImage","image","image_url","imageUrl"];var direct=getImageByKeys(obj,mainKeys);if(direct)return direct;var arrayKeys=["images","product_images","productImages","gallery","media","photos","pictures"];var fromArrays=getBestImageFromArrays(obj,arrayKeys);if(fromArrays)return fromArrays;var weakKeys=["thumbnail","thumbnail_url","thumbnailUrl","thumb","poster"];return getImageByKeys(obj,weakKeys)}
function getImageByKeys(obj,keys){for(var i=0;i<keys.length;i++){var value=getDeepValueByKey(obj,keys[i]);var image=getImageUrlFromValue(value);if(image)return image}return ""}
function getBestImageFromArrays(obj,keys){for(var i=0;i<keys.length;i++){var arr=getDeepValueByKey(obj,keys[i]);if(!Array.isArray(arr)||!arr.length)continue;var image=getBestImageFromList(arr);if(image)return image}return ""}
function getBestImageFromList(list){var fallback="";for(var i=0;i<list.length;i++){var item=list[i];if(isVideoMediaObject(item))continue;var image=getImageUrlFromValue(item);if(!image)continue;if(!isWeakImageUrl(image))return image;if(!fallback)fallback=image}return fallback}
function getImageUrlFromValue(value){if(!value)return "";if(typeof value==="string")return cleanImageUrl(value);if(typeof value!=="object")return "";if(isVideoMediaObject(value))return "";var keys=["url","src","original_url","originalUrl","large","large_url","largeUrl","medium","medium_url","mediumUrl","path","secure_url","secureUrl"];for(var i=0;i<keys.length;i++){var v=value[keys[i]];if(typeof v==="string"){var image=cleanImageUrl(v);if(image)return image}if(v&&typeof v==="object"){var nested=getImageUrlFromValue(v);if(nested)return nested}}return ""}
function getSafeMetaImage(doc){var selectors=['meta[property="og:image:secure_url"]','meta[property="og:image"]','meta[name="twitter:image"]','meta[itemprop="image"]'];for(var i=0;i<selectors.length;i++){var image=cleanImageUrl(getMetaContent(doc,selectors[i]));if(image&&!isWeakImageUrl(image))return image}for(var j=0;j<selectors.length;j++){var fallback=cleanImageUrl(getMetaContent(doc,selectors[j]));if(fallback)return fallback}return ""}
function isVideoMediaObject(obj){if(!obj||typeof obj!=="object")return false;var keys=["type","media_type","mediaType","resource_type","resourceType","mime","mime_type","mimeType","content_type","contentType"];for(var i=0;i<keys.length;i++){var value=String(obj[keys[i]]||"").toLowerCase();if(value.indexOf("video")>-1||value.indexOf("youtube")>-1||value.indexOf("vimeo")>-1)return true}return false}
function cleanImageUrl(value){var src=String(value||"").trim();if(!src)return "";if(src.indexOf(",")>-1&&src.indexOf(" ")>-1)src=getSrcFromSrcset(src);src=src.split(" ")[0].trim();if(!src)return "";var absolute=abs(src);if(isBadMediaUrl(absolute))return "";return absolute}
function getSrcFromSrcset(srcset){var parts=String(srcset||"").split(","),best="";for(var i=0;i<parts.length;i++){var src=parts[i].trim().split(/\s+/)[0];if(src)best=src}return best}
function isBadMediaUrl(url){var u=String(url||"").toLowerCase();if(!u)return true;if(u.indexOf("youtube")>-1||u.indexOf("youtu.be")>-1||u.indexOf("vimeo")>-1)return true;if(u.indexOf(".mp4")>-1||u.indexOf(".webm")>-1||u.indexOf(".mov")>-1||u.indexOf(".m4v")>-1||u.indexOf(".m3u8")>-1)return true;if(u.indexOf("/video")>-1||u.indexOf("video/")>-1||u.indexOf("type=video")>-1||u.indexOf("resource_type=video")>-1)return true;return false}
function isWeakImageUrl(url){var u=String(url||"").toLowerCase();return u.indexOf("thumbnail")>-1||u.indexOf("thumb")>-1||u.indexOf("poster")>-1||u.indexOf("preview")>-1||u.indexOf("placeholder")>-1||u.indexOf("fallback")>-1}
function hasVideoParent(el){var n=el;while(n&&n.nodeType===1){var tag=String(n.tagName||"").toLowerCase(),cls=String(n.className||"").toLowerCase(),id=String(n.id||"").toLowerCase();if(tag==="video"||tag==="iframe")return true;if(cls.indexOf("video")>-1||cls.indexOf("youtube")>-1||cls.indexOf("vimeo")>-1||id.indexOf("video")>-1||id.indexOf("youtube")>-1||id.indexOf("vimeo")>-1)return true;n=n.parentElement}return false}
function getFinalPriceFromProductSlug(doc,slug,productObject){var fromObject=getPriceFromProductObject(productObject);if(fromObject)return fromObject;var fromVisibleDom=getPriceFromVisibleProductDom(doc);if(fromVisibleDom)return fromVisibleDom;return ""}
function getPriceFromProductObject(obj){if(!obj||typeof obj!=="object")return "";var priority=["sale_price","salePrice","current_price","currentPrice","final_price","finalPrice","price_after_discount","priceAfterDiscount","discounted_price","discountedPrice","price"];for(var i=0;i<priority.length;i++){var direct=getDeepValueByKey(obj,priority[i]);var cleaned=formatPrice(direct);if(cleaned)return cleaned}var collected=[];collectPriceValues(obj,collected);for(var j=0;j<collected.length;j++){var price=formatPrice(collected[j]);if(price)return price}return ""}
function getDeepValueByKey(obj,keyName){if(!obj||typeof obj!=="object")return "";if(Object.prototype.hasOwnProperty.call(obj,keyName))return obj[keyName];if(Array.isArray(obj)){for(var i=0;i<obj.length;i++){var foundArr=getDeepValueByKey(obj[i],keyName);if(foundArr)return foundArr}return ""}for(var key in obj){if(Object.prototype.hasOwnProperty.call(obj,key)){var child=obj[key];if(child&&typeof child==="object"){var found=getDeepValueByKey(child,keyName);if(found)return found}}}return ""}
function collectPriceValues(obj,out){if(!obj||typeof obj!=="object")return;if(Array.isArray(obj)){for(var i=0;i<obj.length;i++)collectPriceValues(obj[i],out);return}for(var key in obj){if(!Object.prototype.hasOwnProperty.call(obj,key))continue;var value=obj[key],lower=String(key).toLowerCase(),isPriceKey=lower.indexOf("price")>-1,isBadKey=lower.indexOf("compare")>-1||lower.indexOf("old")>-1||lower.indexOf("original")>-1||lower.indexOf("before")>-1||lower.indexOf("save")>-1||lower.indexOf("discount_percent")>-1||lower.indexOf("percentage")>-1;if(isPriceKey&&!isBadKey&&(typeof value==="string"||typeof value==="number"))out.push(value);if(value&&typeof value==="object")collectPriceValues(value,out)}}
function getPriceFromVisibleProductDom(doc){var selectors=[".ab-pd-price","[class*='ab-pd-price']","[class*='pd-price']",".sale-price",".current-price",".product-price","[class*='sale'][class*='price']","[class*='current'][class*='price']","[class*='final'][class*='price']","[data-price]"];for(var i=0;i<selectors.length;i++){var elements=doc.querySelectorAll(selectors[i]);for(var j=0;j<elements.length;j++){var attr=elements[j].getAttribute("data-price")||elements[j].getAttribute("content")||"",fromAttr=formatPrice(attr);if(fromAttr)return fromAttr;var text=(elements[j].innerText||elements[j].textContent||"").trim(),fromText=extractFirstValidPriceFromText(text);if(fromText)return fromText}}return ""}
function extractFirstValidPriceFromText(text){text=normalizeSpace(text);if(!text)return "";var matches=text.match(/[0-9][0-9,.\s]*/g)||[];for(var i=0;i<matches.length;i++){var price=formatPrice(matches[i]);if(price)return price}return ""}
function cleanPriceCandidate(value){var text=String(value||"").replace(/[^\d.,\s]/g,"").replace(/\s+/g,"").trim();if(!text)return "";if(!/[1-9]/.test(text))return "";var match=text.match(/[0-9][0-9,.]*/);if(!match)return "";var number=match[0];if(number.indexOf(",")>-1&&number.indexOf(".")>-1){number=number.replace(/,/g,"")}else if(number.indexOf(",")>-1){number=number.replace(/,/g,"")}var numeric=parseFloat(number);if(!isFinite(numeric))return "";if(numeric>200000&&numeric<=20000000)numeric=numeric/100;if(numeric<10||numeric>200000)return "";return String(Math.round(numeric))}
...
function getImageFromPage(doc){var fromEasyOrdersGallery=getEasyOrdersMainGalleryImage(doc);if(fromEasyOrdersGallery)return fromEasyOrdersGallery;var selectors=["img#gallery-main-image.ab-gallery-main-media","img#gallery-main-image",".ab-gallery-slide.is-active img.ab-gallery-main-media",".ab-gallery-slide.is-active img","[data-product-image] img","[data-main-image] img","[data-featured-image] img",".ab-gallery-main img",".product-gallery img",".product__media img",".product img",".product-page img",".swiper-slide img","img[src*='digitaloceanspaces']","img"];var fallback="";for(var i=0;i<selectors.length;i++){var imgs=doc.querySelectorAll(selectors[i]);for(var j=0;j<imgs.length;j++){var image=getImageFromImgElement(imgs[j]);if(!image)continue;if(!isWeakImageUrl(image))return image;if(!fallback)fallback=image}}return fallback}
...
function regEsc(value){return String(value||"").replace(/[.*+?^${}()|\[\]\\]/g,"\\$&")}
function markImportant(value){var words=["كرياتين","Creatine","واي بروتين","Whey","ماس جينر","Mass Gainer","MassGainer","بروتين","Protein","مكمل","مكملات","3 أسئلة","45 ثانية","قسيمة","خصم","هدية","معلومة","صحة أفضل","نشر الوعي","حارق دهون","Fat Burner","سترولين","Citrulline","Pump","طاقة","تحمل","ضخ","الضخامة","زيادة الوزن","التخسيس","التنشيف","السعر","المنتج","المنتجات","100 ج","50 ج","RandomSale","100EGP","50EGP"];var safe=esc(value);words.sort(function(a,b){return b.length-a.length});for(var i=0;i<words.length;i++){var w=esc(words[i]);safe=safe.replace(new RegExp("("+regEsc(w)+")","gi"),'<span class="grn">$1</span>')}return safe}
function findShippingTarget(){
  var selectors=[
    ".p_details_container .bg-white.rounded-lg.border.border-gray-300.mt-8.max-w-full",
    ".p_details_container .bg-white.rounded-lg.border.border-gray-300",
    ".p_details_container [class*='border-gray-300'][class*='mt-8']",
    ".p_details_container [class*='border'][class*='rounded'][class*='bg-white']"
  ];
  ...
  var container=document.querySelector(".p_details_container")||document.querySelector("main")||document.body;
  var words=["shipping","الشحن","شحن","التوصيل","توصيل","الدفع عند الاستلام","استلام"];
  var candidates=container?container.querySelectorAll("div,section,article,aside"):[];

  for(var j=0;j<candidates.length;j++){
    var text=normalizeSpace(candidates[j].textContent||"").toLowerCase();
    if(!text)continue;

    var match=false;
    for(var k=0;k<words.length;k++){
      if(text.indexOf(words[k].toLowerCase())>-1){
        match=true;
        break;
      }
    }
    ...
    if(match){
      var card=candidates[j].closest(".bg-white.rounded-lg.border")||candidates[j].closest("[class*='border'][class*='rounded']")||candidates[j];
      if(card&&card.id!=="pqx"&&!card.closest("#pqx")&&!card.closest("#pqo"))return card;
    }
  }
...
function placeGameUnderShipping(){
  ...
  var target=findShippingTarget();
  if(!target||!target.parentNode)return false;

  var root=G("pqx")||document.createElement("div");
  root.id="pqx";

  if(root.parentNode!==target.parentNode||root.previousElementSibling!==target){
    target.insertAdjacentElement("afterend",root);
  }

  if(!root.getAttribute("data-ready")||!root.innerHTML.trim()){
    inlineCard();
...
function observeShippingTarget(){
  if(GAME_STATE.observer){
    GAME_STATE.observer.disconnect();
    GAME_STATE.observer=null;
  }

  var queued=false;
  var run=function(){
    queued=false;
    placeGameUnderShipping();
  };

  var observer=new MutationObserver(function(){
    if(queued)return;
    queued=true;
    (window.requestAnimationFrame||function(cb){return setTimeout(cb,60)})(run);
  });

  if(document.body){
    observer.observe(document.body,{childList:true,subtree:true});
    GAME_STATE.observer=observer;
  }
}

function watchRouteChanges(){
  if(GAME_STATE.routeWatcher)return;
  GAME_STATE.routeWatcher=true;

  var trigger=function(){
    setTimeout(function(){mount()},80);
    setTimeout(function(){mount()},600);
    setTimeout(function(){mount()},1500);
  };
...
function mount(){
  placeGameUnderShipping();
  observeShippingTarget();
  watchRouteChanges();

  if(GAME_STATE.autoTimer)clearTimeout(GAME_STATE.autoTimer);
  GAME_STATE.autoTimer=setTimeout(function(){
    if(/\/products\//.test(location.pathname)&&!auto&&!G("pqo")){
      auto=1;
      intro();
    }
  },DELAY);
